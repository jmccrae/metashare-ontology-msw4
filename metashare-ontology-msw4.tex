...
al.~\cite{Villegas2014} for the University Pompeu Fabra's (UPF) META-SHARE node, covering part of the original schema, however extending this initial effort to the whole schema and all LRs and incorporating the consensus
...
Our approach has several advantages. Firstly, the use of Semantic Web techniques enables standardized means of representing, linking, and accessing the data, while 
...
\item components specific to the resource type (corpus, lexical/conceptual
    ...
    image), which allow the encoding of information relevant to resource/media combinations, e.g. text or audio parts of corpora, lexical/conceptual resources etc., such as language, formats, classification.
\end{itemize} 
...
principle may derive an unnecessarily verbose graphs. Thus,
...
cases. DCAT's {\tt dataset} corresponds nearly exactly to the {\tt resourceInfo} tag and similarly, {\tt distributions} are similar to {\tt distributionInfo} 
classes and {\tt catalogRecord} is similar to {\tt metadataInfo}, so we
...
be in English, but the subtitles can be in French and German.
...
providing a range of recommended standard licenses organised on the following axes: open licences are the preferred option (Creative Commons licences for data resources and Free Open Source Software for tools and
...
custom and proprietary licences are also allowed for legacy
...
creating RDF codes for non standard licences: common terms and conditions (e.g. attribution) are mapped to ODRL
actions (duty to attribute), and included in an RDF
document, in which they are ready to be complemented by other
...
specific language resource metadata. The most frequent 'Song' tag focuses on
...
describing the participants in a media recording. 
...
paper are being integrated into the system to allow users to use META-SHARE as the
...
such as the one presented in paper should enable the emergence of this
paradigm and thus contribute to the development of novel NLP algorithms and
